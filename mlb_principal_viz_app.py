...
# ───────────────── BEST‑ODDS LOOKUP ─────────────────
# (same as previous but moved below for brevity)
from functools import lru_cache

@lru_cache(maxsize=1024)
def _futures_table(event_type: str, event_label: str):
    mapping = {
...
def best_odds_decimal_prob(event_type, event_label, participant, cutoff_dt, fut_conn):
    tbl = _futures_table(event_type, event_label)
    if not tbl:
        return 1.0, 0.0
...
def ev_table_page():
    ...
    st.caption(
        "Active stake + expected payout + realized profit → **Expected Value** (per market)"
    )
