...
if page == "GreenAleph Active Principal":
    ...
    if data is None or deployed_data is None:
        ...
        if deployed_data and len(deployed_data) > 0 and 'TotalDollarsDeployed' in deployed_data[0]:
            ...
            st.markdown(f"<h3 style='text-align: center; font-weight: bold; color: black;'>Total Deployed (Total Active Principal minus Realized Profit)</h3>", fontsize = 14, unsafe_allow_html=True)
            st.markdown(f"<h4 style='text-align: center; font-weight: bold; color: gray;'>$500k Initial Deployment Goal</h4>", fontsize = 12, unsafe_allow_html=True)
