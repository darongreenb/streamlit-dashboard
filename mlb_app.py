...
def with_cursor(conn):
    """Context manager that pings & always closes the cursor."""
...
def best_odds_decimal_prob(event_type,event_label,participant,cutoff_dt,fut_conn):
    """Most‑recent non‑zero odds ≤ cutoff"""
...
def return_plot_page_fast():
    ...
    # Pre‑compute weight per leg for *selected* market
    wgt = (bet_df
    ...
    if wgt.empty:
        st.warning("No active legs match selected market"); return

    # ---- 2) Bulk‑load odds snapshots (vectorised) ----
    ...
    def cached_odds(tbl, names, start, end):
        ...
        # keep latest snapshot per team per day
        return (raw
    ...
    # ---- 3) Assemble daily return (vectorised) ----
    ...
    merged = merged.merge(wgt, on="WagerID", how="inner")   # only keep selected‑market legs

    # probability product per wager per day (here only one leg per wager after merge)
