...
if page == "GreenAleph Active Principal":
    ...
    # SQL query to fetch data for the first bar chart (Total Active Principal by EventType)
    first_chart_query = """
    WITH DistinctBets AS (
    ...
    if first_chart_data is None:
        ...
        if 'TotalDollarsAtStake' not in first_chart_df.columns:
            ...
            fig, ax = plt.subplots(figsize=(15, 10))
            ax.bar(first_chart_df['EventType'], first_chart_df['TotalDollarsAtStake'], width=0.6, edgecolor='black')
            ...
            # Annotate each bar with the value
            for i, v in enumerate(first_chart_df['TotalDollarsAtStake']):
                ax.text(i, v + 500, f'{v:,.0f}', ha='center', va='bottom', fontsize=12)
            ...
            if event_type_option:
                ...
                if event_label_data is None:
                    ...
                    if event_label_option:
                        # SQL query to fetch Participant data for the combined bar chart (DollarsAtStake and PotentialPayout)
                        combined_query = f"""
                        WITH DistinctBets AS (
                        ...
                        if combined_data is None:
                            ...
                            # Check if 'TotalDollarsAtStake' exists in the DataFrame
                            if 'TotalDollarsAtStake' not in combined_df.columns:
                                st.error("'TotalDollarsAtStake' column not found in the DataFrame.")
                            else:
                                # Modify to multiply TotalDollarsAtStake by -1 for chart (to show negative values)
                                ...
                                ax.barh(combined_df['ParticipantName'], combined_df['TotalDollarsAtStake'], 
                                        color='lightblue', edgecolor='black', label='Active Principal')

                                # Plot TotalPotentialPayout moving upward from the x-axis
                                ax.barh(combined_df['ParticipantName'], combined_df['TotalPotentialPayout'], 
                                        color='beige', edgecolor='black', label='Potential Payout')

                                # Add labels and title
                                ax.set_xlabel('USD ($)', fontsize=16, fontweight='bold')
                                ax.set_title(f'Active Principal & Potential Payout by Participant for {event_type_option} - {event_label_option}', fontsize=18, fontweight='bold')

                                # Annotate the chart with values
                                for i, v in enumerate(combined_df['TotalDollarsAtStake']):
                                    ax.text(v - 500, i, f'{abs(v):,.0f}', ha='center', va='center', fontsize=12, color='black')
                                for i, v in enumerate(combined_df['TotalPotentialPayout']):
                                    ax.text(v + 500, i, f'{v:,.0f}', ha='center', va='center', fontsize=12, color='black')
