...
def with_cursor(conn):
    """Ping + context-managed cursor (auto-close)."""
...
# Full market team names (as in betting DB) → alias (as in futures DB)
team_alias_map = {
    "Philadelphia 76ers":"76ers","Milwaukee Bucks":"Bucks","Chicago Bulls":"Bulls",
    "Cleveland Cavaliers":"Cavaliers","Boston Celtics":"Celtics",
    "Los Angeles Clippers":"Clippers","Memphis Grizzlies":"Grizzlies","Atlanta Hawks":"Hawks",
    "Miami Heat":"Heat","Charlotte Hornets":"Hornets","Utah Jazz":"Jazz","Sacramento Kings":"Kings",
...
def best_odds_decimal_prob(event_type,event_label,participant,cutoff_dt,fut_conn):
    """Return (decimal_odds, implied_prob) for the best non-zero odds available."""
    table = futures_table_map.get((event_type, event_label))
    if not table: return 1.0, 0.0
    alias = team_alias_map.get(participant, participant)   # player stays same
...
def ev_table_page():
    ...
    for wid,legs in wager_legs.items():
        ...
        if sum_exc<=0: continue
        for d,et,el in decs:
            realised_np[(et,el)] += net*((d-1)/sum_exc)

    bet_conn.close(); fut_conn.close()

    # --- Table
    rows_out=[]
    keys=set(active_stake)|set(active_exp)|set(realised_np)
    for et,el in sorted(keys):
        stake=active_stake.get((et,el),0); exp=active_exp.get((et,el),0); net=realised_np.get((et,el),0)
        rows_out.append({
    ...
    st.dataframe(pd.DataFrame(rows_out).sort_values(["EventType","EventLabel"]),
...
def return_plot_page_fast():
    ...
    with with_cursor(bet_conn) as cur:
        cur.execute("""
            SELECT b.WagerID,b.PotentialPayout,b.DollarsAtStake,b.DateTimePlaced,
             ...
             WHERE b.WhichBankroll='GreenAleph' AND b.WLCA='Active' AND l.LeagueName='NBA'
        """)
        bet_df = pd.DataFrame(cur.fetchall())
    if bet_df.empty: st.warning("No active wagers"); return

    # alias column (team→alias, player unchanged)
    bet_df["Alias"] = bet_df["ParticipantName"].apply(lambda x: team_alias_map.get(x, x))

    # keep only wagers that include selected market (weight = 1/leg_count)
    wgt = (bet_df
    ...
    st.caption(f"{len(wgt)} active wagers include that market.")
    if wgt.empty: return

    participants = bet_df.loc[bet_df["Alias"].isin(wgt["WagerID"].map(
        bet_df.set_index("WagerID")["Alias"])),"Alias"].unique().tolist()
        ...
        with with_cursor(fut_conn) as cur:
            cur.execute(
                f"""SELECT team_name,date_created,{','.join(sportsbook_cols)}
                      FROM {tbl}
                     WHERE team_name IN ({placeholders}) AND date_created BETWEEN %s AND %s
    ...
    if odds_df.empty: st.warning("No odds data for that period"); return

    merged = (bet_df
              .merge(odds_df, how="left",
                     left_on="Alias", right_on="team_name"))
    ...
    daily = (merged.groupby(["date","WagerID"])
                   .agg(prob=("prob","prod"),
                        weight=("Weight","first"))
                   .reset_index())
    meta = bet_df[["WagerID","PotentialPayout","DollarsAtStake"]].drop_duplicates()
    daily = daily.merge(meta, on="WagerID")
    for c in ["weight","DollarsAtStake","PotentialPayout"]:
...
(ev_table_page if page == "EV Table" else return_plot_page_fast)()
