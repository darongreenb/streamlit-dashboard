            ...
            if event_label_data is None:
                ...
                if event_label_option:
                    ...
                    if combined_data is None:
                        ...
                        # Modify the second bar chart to multiply TotalDollarsAtStake by -1
                        combined_df['TotalDollarsAtStake'] = -combined_df['TotalDollarsAtStake']  # Multiply by -1 to flip the values below y=0

                        # Sort the DataFrame by 'TotalDollarsAtStake' in ascending order again (since values are now negative)
                        ...
                        color_dollars_at_stake = '#219ebc'  # Blue color for Dollars At Stake (negative)
                        color_potential_payout = '#f4a261'  # Light orange for Potential Payout
                        ...
                        # Annotate each bar with the TotalDollarsAtStake value below the bar (since it's negative now)
                        for bar1 in bars1:
                            height = bar1.get_height()
                            ax.annotate(f'{abs(height):,.0f}', xy=(bar1.get_x() + bar1.get_width() / 2, height),
                                        xytext=(0, -10), textcoords="offset points",  # Move the text below the negative bar
                                        ha='center', va='bottom', fontsize=12, fontweight='bold', color='black')

                        # Annotate each bar with the TotalPotentialPayout value above the orange bar
                        for bar2 in bars2:
            ...
            if event_label_data is None:
                ...
                if event_label_option:
                    ...
                    if combined_data is None:
                        ...
                        # Apply the -1 to TotalDollarsAtStake
