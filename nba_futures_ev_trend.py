...
st.set\_page\_config(page\_title="NBA Futures Probabilities", layout="wide")
...
def american\_odds\_to\_probability(odds: int) -> float:
if odds > 0:
...
if **name** == "**main**":
main()" modify this code to autoscale the y axis. the top value should not always be 100%, instead it should be slightly higher than the highest any line on the plot goes. eg if all lines on the plot are around 5% and below, then the y axis should be rescaled so its easier to visualize those lower values
